...
path_list = readRDS(here("data","CPLASS_MTEB1_1st_1.rds"))
#######################
...
# outfile = paste0(filestub,"_gallery_1.pdf")
...
path_list = readRDS(here("data","CPLASS_MTEB1_2nd_1.rds"))
#######################
# for REAL DATA use the lines below.
...
real_data_not_merged_file = c(here("data","CPLASS_MTEB1_1st_1.rds"))
real_data_merged_file = c(here("data","CPLASS_MTEB1_2nd_1.rds"))
...
p_csa = ggplot(csa %>% filter(label == cohort_list[1] | 
  ...
  ggtitle("MTEB1")+
  ylab("Cumulative Speed Allocation")+
  xlab("Speed (unknown unit)")+
# +xlab(expression(paste("Speed (",mu,"m/sec)")))+
...
ggplot(segment_summary, aes(x = speeds, fill = label)) +
  ...
  labs(title = "KDE of inferred speed in different approaches", x = "Speed (unknown unit)", y = "Density") +
  theme_minimal()
```

From the CSA plot, looking at the speed between 0 and 5, the blobs in the second approach spend more time at these speed values than those in the first approach.  

Merged tracks in the second approach provide a clearer picture of where a blob passes branch points, such as the paths we plotted above.
